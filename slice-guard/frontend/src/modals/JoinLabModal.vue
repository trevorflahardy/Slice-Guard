...
<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="emit('close')">
      <div class="bg-surface-lowest p-6 rounded-xl w-80">
        <h2 class="text-lg mb-3 text-fg-primary">Join a Lab</h2>
        <input v-model="code" type="text" placeholder="Invite Code" @keyup.enter="join"
          class="w-full p-2 rounded bg-surface-high text-fg-primary" />
        <p v-if="status==='invalid'" class="text-red-500 text-sm mt-1">Invalid code</p>
